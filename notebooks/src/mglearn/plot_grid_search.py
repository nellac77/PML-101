...
def plot_cross_val_selection():
    ...
    grid_search = GridSearchCV(SVC(), param_grid, cv=5, return_train_score=True, iid=False)
    ...
    marker_cv, = plt.plot(scores.mean_train_score + scores.std_train_score, '^', c='gray', markersize=5, alpha=.5)
    marker_cv, = plt.plot(scores.mean_train_score - scores.std_train_score, '^', c='gray', markersize=5, alpha=.5)

    marker_mean, = plt.plot(scores.mean_train_score, 'v', c='none', alpha=1, markersize=10)
    i = np.where(scores.rank_test_score == 1)[0]
    best = scores.iloc[i]
    marker_best, = plt.plot(i, best.mean_train_score, 'o', c='red', fillstyle="none", alpha=1, markersize=20, markeredgewidth=3)

    plt.xticks(range(len(scores)), [str(scores.iloc[idx].params).strip("{}").replace("'", "") for idx in range(scores.shape[0])], rotation=90);
